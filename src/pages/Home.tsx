...
const featuredBuilds = [
  {
    ...
    category: 'Applied AI',
    outcome:
      'Natural-language analytics over local SQLite with schema RAG, SQL validation, and privacy-aware execution.',
  ...
  },
  {
    ...
    outcome:
      'Calibrated food recognition with confidence routing, multi-food detection, and full-stack delivery.',
  ...
  },
  {
    ...
    outcome:
      'Sydney Airbnb and Census warehouse with orchestration, dimensional marts, and SCD Type 2 history.',
    ...
    <div className="space-y-16">
      ...
      <section className="grid min-h-[66vh] min-w-0 items-center gap-10 py-8 lg:grid-cols-[1.08fr_0.92fr]">
        ...
        >
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <span className="section-eyebrow">Open to ML and data engineering roles</span>
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-400 sm:text-lg md:text-xl">
            ...
            building AI agents, deployable models, and data pipelines that are easy to inspect,
            rerun, and trust.
          </p>
          ...
          <div className="soft-panel grid gap-3 p-3 sm:grid-cols-3">
            {focusAreas.map((area) => (
              <div
                key={area.title}
                className="rounded-md bg-white/80 px-3 py-3 dark:bg-slate-900/70"
              >
      ...
      <section className="soft-panel grid md:grid-cols-[0.9fr_1.1fr] gap-10 items-center p-6 md:p-8">
        ...
        <div className="space-y-4 text-base md:text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
          <p>
            My background spans consulting, audit, and e-commerce analytics. That shapes how I
            build: clear business framing, reliable data foundations, and outputs people can act on.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="space-y-3">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-2xl">
            Practical principles from moving between business problems, data systems, and ML
            delivery.
          </p>
        </div>
            ...
            >
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <h3 className="text-xl font-bold">{reflection.title}</h3>
                </div>
                ...
                <ul className="space-y-2">
                  {reflection.points.map((point: string, i: number) => (
                    <li
                      key={i}
                      className="text-slate-600 dark:text-slate-400 leading-relaxed text-xs flex gap-2"
                    >
      ...
      <section className="py-14 space-y-8">
        <div className="max-w-3xl space-y-3">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400">
            Tools and patterns I use to move from raw data to reliable outputs.
          </p>
        </div>
          ...
          >
            <div className="max-w-3xl space-y-3">
              ...
              <h3 className="text-3xl md:text-4xl font-black tracking-tight">
                From raw data to systems people can trust.
              </h3>
              <p className="text-lg leading-relaxed text-slate-500 dark:text-slate-400">
                The stack is organized around delivery: prepare reliable data, model with evidence,
                ship usable interfaces, and make every output easy to review.
              </p>
            </div>

            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {lifecycleStack.map((group) => (
                <div key={group.step} className="surface-card surface-card-hover p-5">
                  <div className="flex items-start gap-3">
                    <div className="icon-tile mt-1 h-9 w-9">{group.icon}</div>
                    <div>
